...
//   3. **El ancla mueve `sy` y jamás `sx`.** El centrado horizontal casi nunca
//      se equivoca; el vertical corta cabezas, y para eso está el ancla.
//   4. **Sin `ratio` no se recorta nada**, sólo se achica. Es el modo de toda
//      subida.
//   5. Una foto que no cargó (0×0) no puede producir un lienzo `NaN`.
import { encuadre, FORMATOS, ANCHO_MAX } from "../lib/imagenes-encuadre";
...
console.log("\n4. El ancla mueve el eje vertical y NADA más");
{
  // Foto vertical llevada a 16:9: sobra alto ⇒ el ancla decide.
  const arriba = encuadre(1000, 2000, FORMATOS["16:9"].ratio, ANCHO_MAX, "arriba");
  const centro = encuadre(1000, 2000, FORMATOS["16:9"].ratio, ANCHO_MAX, "centro");
  const abajo = encuadre(1000, 2000, FORMATOS["16:9"].ratio, ANCHO_MAX, "abajo");
  ok(arriba.sy === 0, "arriba arranca en 0", String(arriba.sy));
  ok(centro.sy > arriba.sy && centro.sy < abajo.sy, "centro queda en el medio", `${arriba.sy} < ${centro.sy} < ${abajo.sy}`);
  ok(abajo.sy + abajo.sh === 2000, "abajo termina justo en el borde", `${abajo.sy}+${abajo.sh}`);
  ok(
    arriba.sx === centro.sx && centro.sx === abajo.sx,
    "y el eje horizontal no se movió con ninguna",
    `${arriba.sx} ${centro.sx} ${abajo.sx}`,
  );
  // Cuando lo que sobra es ANCHO, el ancla no tiene nada que decir.
  const a2 = encuadre(4000, 1000, FORMATOS["1:1"].ratio, ANCHO_MAX, "arriba");
  const b2 = encuadre(4000, 1000, FORMATOS["1:1"].ratio, ANCHO_MAX, "abajo");
  ok(a2.sy === b2.sy && a2.sx === b2.sx, "sobrando ancho, el ancla no cambia nada");
  ok(a2.sx > 0, "y el corte lateral va centrado", String(a2.sx));
}

console.log("\n5. Sin formato no se recorta: sólo se achica");
{
...
console.log("\n6. Una foto que no cargó no produce un lienzo NaN");
{
