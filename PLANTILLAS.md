...
| foto más chica que la tarjeta, centrada o a un costado | ✅ | `imagen.ancho` (% del ancho útil) + `imagen.align`, desde el 11-ago-2026. 🔑 **El % se resuelve a PÍXELES y sale también como atributo `width`**: Outlook ignora `max-width` y no escala una imagen por CSS, así que un `width:50%` le dibujaba la foto a tamaño natural — mismo camino que `logoAncho`. La alineación va como `text-align` de un `<div>` envolvente con la foto en `inline-block` (`margin:0 auto` no centra nada en Word), que es la forma del ícono de celda. ⛔ **Sin media query de rescate**: en el celular el `max-width:100%` acota sola a la que no entra —medido a 390 px: la de 75% queda en 316 y la de 33% en 177— y una clase que forzara el 100% haría que el editor mienta justo en el aparato donde se abre casi todo. `sangre` le gana, y el clamp (25-100) vive en el renderer porque `sanearBloque` deja pasar las claves que no conoce | — (salió de un pedido de uso real, no de una referencia) |
...
| color de fondo en el **menú** | ✅ | `caja.fondo` en `menu` desde el 3-ago-2026: **con `fondo` elegido el bloque deja de pasar por `pad()` y dibuja su propio contenedor**, que es el camino de `hero` y `seccion`. Sin `fondo` sale byte por byte como salía —el golden no se movió— y el `margin:12px 0` de siempre pasó a ser `padY ?? 12`. 🔑 **Con banda el aire va ADENTRO** (`padding`): un margen dejaría una franja del fondo de página entre el encabezado y el menú, que es justo lo que esas referencias tienen pegado. Los links se recalculan contra la banda si nadie eligió el color (sobre `#111111`: `#171717` → `#d5d4d4`). ⛔ `align` **no** entró en la caja: la alineación de la barra la gobierna `cuerpo.align` y dos perillas para lo mismo es lo que documenta `SIN_EFECTO`. ✅ **Aplicado a los presets el 3-ago-2026**, con la captura al lado: `ocasion` `#fcf0ec` · `dos-colores` `#000000` · `final-sale` `#18a8e8` · `vuelta-al-cole` `#000000` · `negro-y-dorado` `#000000` (el único que cierra el mail en vez de abrirlo). 🔴 **Son CINCO y no seis: R-021 pone el menú sobre BLANCO** —entre la banda oscura del logo y la portada negra—, así que `mega-oferta` se queda sin banda a propósito y sale por el camino viejo | 003 · 005 · 010 · 014 · 016 · ~~021~~ |
...
| precio más grande que el nombre en la tarjeta | ✅ | **rol `precio` desde el 5-ago-2026**: hasta ese día el precio era HTML escrito a mano y su tamaño salía del rol `cuerpo`, o sea del **nombre** del producto. El BASE del rol nuevo es idéntico al hardcode viejo, y el precio **tachado** pasó a consumir `nota` (cuyo BASE ya era ese hardcode). De yapa entró `precioOculto` | las 7 de catálogo |
| grilla de 4 por fila | ✅ | **`porFila: 4` desde el 6-ago-2026**. La celda queda en 134px (118 de foto) y ahí el nombre en el BASE de 14px entra en dos renglones y el precio con el tachado en uno — medido en Chrome sobre el render real. 🔑 **Con cuatro el botón por tarjeta NO se dibuja**: necesita 147px contra 118 disponibles y sale partido en dos renglones ("COMP / RAR"), que es el defecto que tienen las capturas mismas. El `botonTexto` **no se borra**: bajar a 3 o a 2 lo devuelve. ⚠️ **No cuesta una llamada más a TN**: `claveProductos` es `fuente\|categoriaId\|n` y `porFila` no entra en la llave | 004 · 007 · 011 · 021 |
...
| ✅ 7 | **precio más grande que el nombre** | **hecho el 5-ago-2026** (commit `80c4163`), lo pedían las siete referencias de catálogo: rol `precio` en la cascada, con BASE idéntico al HTML que `precioHtml` escribía a mano ⇒ el golden se movió en **2 de las 38** plantillas —marroquinería y joyería, **un hunk cada una**: el gris del precio **tachado**, que las dos ya elegían en `nota` a nivel documento y el tachado ignoraba—. 🔑 **La advertencia que estaba acá era falsa**: `ROLES_POR_TIPO` es una lista **positiva**, así que un rol nuevo se declara sólo en los tipos que lo usan (`productos`, `productos-dinamicos`, `carrito`) y **no obliga a ninguna entrada de `SIN_EFECTO`** — lo que sí hay que tocar es `PROPS_POR_ROL`. Un rol nuevo cuesta menos de lo que esta fila decía |
| ✅ 4 | grilla de 4 por fila | **hecho el 6-ago-2026**: `PorFila` pasó a `2 \| 3 \| 4` y el `GrillaControl` ofrece la tercera opción. El golden **no se movió** —ausente sigue siendo 2 y ningún preset pedía cuatro—. 🔑 Lo que la fila de acá no decía: **el botón por tarjeta no entra a ese ancho** (147px contra 118), así que con cuatro no se dibuja. Lo fija `probar-html.ts` con el `botonTexto` PUESTO, o el test pasaría por la razón equivocada |
...
| ✅ 5 | color de fondo en el **menú** | **hecho el 3-ago-2026** (lo autorizó Bruno cuando el contador llegó a 6): el `case "menu"` dibuja su propio contenedor **solo si alguien eligió `fondo`**; sin fondo sigue en `pad()` y el mail no cambia un byte. 🔑 El aire pasó a `padY ?? 12`, que reproduce el `margin:12px 0` de siempre ⇒ el golden quedó quieto y de yapa el margen se volvió una perilla del panel. ✅ **Aplicado a los presets el mismo día**: `ocasion` `#fcf0ec` (`editorial.ts:64`) · `dos-colores` `#000000` con los links en lima (`editorial.ts:200`) · `final-sale` `#18a8e8` (`venta.ts:136`) · `negro-y-dorado` `#000000` (`producto.ts:217`) · `vuelta-al-cole` `#000000` (`fechas.ts:378`). 🔴 **Son CINCO y no seis**: R-021 pone el menú sobre BLANCO —entre la banda oscura del logo y la portada negra—, así que `mega-oferta` se queda sin banda **a propósito** |
